...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    if A.is_complex():
        ...
        B = B.float()
        C = C.float()
    #初始化x作为存储hiddenstate的容器
    #x = A.new_zeros((batch, dim, dstate))
    #初始化x的时候不让x为0而是让他等于u[1]
    x = u[:, :, 0].unsqueeze(-1).expand(-1, -1, dstate)
    #print("X has been initialized in a new way")
    ys = []
    ...
    last_state = None
    for i in range(u.shape[2]):  #先令x为空，u是实际传输的x值，进行一次后x就可以代表hidden state了
        x = deltaA[:, :, i] * x + deltaB_u[:, :, i] ## h(t + 1) = Ah(t) + Bx(t)
        if not is_variable_C:
            y = torch.einsum('bdn,dn->bd', x, C)  # y(t+1) = Ch(t+1)  (B,D,N)*(B,N)->(B,D)
...
def selective_scan_ref_sub(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
                      return_last_state=False):
    """
    u: r(B D L)
    delta: r(B D L)
    ...
    last_state_sub = None
    for i in range(u.shape[2]):  #先令x为空，u是实际传输的x值，进行一次后x就可以代表hidden state了
        x = deltaA[:, :, i] * x + deltaB_u[:, :, i] ## h(t + 1) = Ah(t) + Bx(t+1)
        if i == 0:
            x_sub = torch.zeros_like(x)  #h(t=0) = 0  注意尺寸x_sub和x一致
        else:
            x_sub = x - deltaB_u[:, :, i]
        if not is_variable_C:
            y = torch.einsum('bdn,dn->bd', x, C)  # y(t+1) = Ch(t+1)  (B,D,N)*(B,N)->(B,D)
            y_sub = torch.einsum('bdn,dn->bd', x_sub, C)  # y(t+1) = C[h(t+1)-Bx(t+1)]  (B,D,N)*(B,N)->(B,D)
        else:
            if C.dim() == 3:
                y = torch.einsum('bdn,bn->bd', x, C[:, :, i])
                y_sub = torch.einsum('bdn,bn->bd', x_sub, C[:, :, i])
            else:
                y = torch.einsum('bdn,bdn->bd', x, C[:, :, :, i])
                y_sub = torch.einsum('bdn,bdn->bd', x, C[:, :, :, i])
        if i == u.shape[2] - 1: #当在序列的最后一个时间步的索引时
            last_state = x
            last_state_sub = x_sub
        if y.is_complex(): #如果 y 是复数张量，则忽略其虚部，只保留实部，并将实部乘以 2。
            y = y.real * 2
        if y_sub.is_complex():
            y_sub = y_sub.real * 2
        ys.append(y)
    ...
    y = torch.stack(ys, dim=2) # (batch dim L)
    y_sub = torch.stack(ys_sub, dim=2)

    out = y if D is None else y + u * rearrange(D, "d -> d 1")
    out_sub = y_sub if D is None else y_sub + u * rearrange(D, "d -> d 1")

    if z is not None:
        out = out * F.silu(z)
        out_sub = out_sub * F.silu(z)
    out = out.to(dtype=dtype_in)
    out_sub = out_sub.to(dtype=dtype_in)

    #return out if not return_last_state else (out, last_state)
    return out_sub if not return_last_state else (out_sub, last_state_sub)
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if D is not None:
            D = D.contiguous()
        #print("preparing for selective_scan_cuda")
        # 
        #print("selective_scan_cuda module imported successfully in forward.")
        #print("selective_scan_cuda is located at:", selective_scan_cuda.__file__)
        out, scan_intermediates, out_z = selective_scan_cuda.fwd(
            conv1d_out, delta, A, B, C, D, z, delta_bias, delta_softplus
        )
        #print("selective_scan_cuda is done!") 
        ctx.delta_softplus = delta_softplus
...
def mamba_inner_fn_no_out_proj(
    ...
    #                           A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
    #print("run mamba_inner_fn_no_out_proj")
    return mamba_inner_fn_no_out_proj(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
...
def bimamba_inner_ref(
    ...
    # 调整卷积权重的形状
    conv1d_weight = rearrange(conv1d_weight, "d 1 w -> d w")
    x, z = xz.chunk(2, dim=1)  # 将输入拆分为 x 和 z 两部分
    conv1d_bias = conv1d_bias.contiguous() if conv1d_bias is not None else None
